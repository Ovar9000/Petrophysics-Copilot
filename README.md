...
[![Plotly](https://img.shields.io/badge/Visualization-Plotly.js-3F4F75.svg?logo=plotly&logoColor=white)](https://plotly.com)
[![Weaviate](https://img.shields.io/badge/Vector%20DB-Weaviate-FA6400.svg?logo=weaviate&logoColor=white)](https://weaviate.io)
[![Gemini](https://img.shields.io/badge/LLM-Google%20Gemini%20API-4285F4.svg?logo=google&logoColor=white)](https://ai.google.dev)
...
The platform is purpose-built to deliver **real-time visualization, retrieval, and computation of well log parameters directly from user requests**:
...
| **Subsurface Retrieval** | Vector RAG + Dynamic LAS depth slicing | • **Tabular Log Retrieval**: Slicing raw `.las` curve arrays on-demand based on requested depth boundaries.<br>• **Semantic Geological RAG**: Vector similarity search (Weaviate) & catalog lookup for formation tops, stratigraphy summaries, and mudlog hydrocarbon show notes. |
...
flowchart TB
    ...
    subgraph Backend["FastAPI Petrophysical Engine (Port 8000)"]
        ...
        Catalog["Stratigraphy & Mudlog RAG\n(Local Catalog / Weaviate DB)"]
    end
...
### 1. The Core Modules
- **ackend/main.py**: High-throughput FastAPI application exposing REST endpoints for chat sessions, individual petrophysical calculations, and dynamic Plotly figure generation.
- **ackend/agent.py**: The agent runtime. Maintains conversational state, formats petrophysical system prompts, sends strict OpenAPI tool schemas to Gemini, dispatches tool executions, and handles multi-turn loops until final synthesis is reached.
- **ackend/petrophysics.py**: Pure, deterministic subsurface math engine. Uses lasio for high-fidelity LAS reading, standardizes mnemonic curve names (e.g. CNC, NPHI -> Neutron; RHOB, DENB -> Bulk Density), executes vector math via 
umpy/scipy, and builds responsive multi-track Plotly figures.
- **ackend/catalog.py**: Semantic metadata and stratigraphy catalog. Indexes geological tops, formation summaries, and hydrocarbon mudlog shows with local caching and Weaviate vector search fallbacks.
- **ackend/mcp_server.py**: FastMCP implementation exposing all petrophysical routines to Claude Desktop, Cursor, or any MCP-compliant sidecar.
...
- **Track 2**: Resistivity curves (RDEEP, RMED, RSHAL) on a 4-decade logarithmic scale (.2\text{--}2000\,\Omega\cdot\text{m}$).
...
| 	op_depth | NUMBER | No | Full log top | Starting depth in meters |
| ottom_depth| NUMBER | No | Full log base| Ending depth in meters |
...
- Overlays Quartz (.65\,\text{g/cm}^3$), Calcite (.71\,\text{g/cm}^3$), and Dolomite (.87\,\text{g/cm}^3$) matrix calibration lines.
...
| 	op_depth | NUMBER | No | None | Top depth boundary in meters |
| ottom_depth| NUMBER | No | None | Bottom depth boundary in meters |
</details>
...
- **Gross Thickness ($)**: Total measured thickness of the interval.
- **Net Reservoir**: Interval meeting Shale Volume cutoff ({\text{sh}} \le 0.30$).
- **Net Pay**: Reservoir meeting Porosity ($\phi_e \ge 0.10$) and Water Saturation ( \le 0.50$) criteria.
- **Net-to-Gross (NTG)**: $\frac{\text{Net Pay}}{\text{Gross}}$.

| Parameter | Type | Required | Default | Description |
| :--- | :--- | :--- | :--- | :--- |
| well_id | STRING | **Yes** | — | Well identifier |
| 	op_depth | NUMBER | **Yes** | — | Top depth in meters |
| ottom_depth| NUMBER | **Yes** | — | Bottom depth in meters |
| sh_cutoff | NUMBER | No |  .30 | Maximum allowable shale volume fraction |
| phi_cutoff | NUMBER | No |  .10 | Minimum allowable effective porosity fraction |
| sw_cutoff | NUMBER | No |  .50 | Maximum allowable water saturation fraction |
</details>
...
| :--- | :--- | :--- | :--- | :--- |
| well_id | STRING | **Yes** | — | Well identifier |
| 	op_depth | NUMBER | No | Full log | Optional top depth boundary |
| ottom_depth| NUMBER | No | Full log | Optional bottom depth boundary |
</details>

<details>
<summary><b>5. <code>plot_3d_wellbore_trajectory</code> — True 3D Spatial Wellbore & Pay Horizon</b></summary>

Renders the true 3D spatial trajectory (, Y, \text{TVDSS}$) using minimum curvature calculation, colored by hydrocarbon pay flags and framed by the regional reservoir top formation surface.

| Parameter | Type | Required | Default | Description |
| :--- | :--- | :--- | :--- | :--- |
| well_id | STRING | **Yes** | — | Well identifier |
| 	op_depth | NUMBER | No | Full log | Interval start |
| ottom_depth| NUMBER | No | Full log | Interval end |
</details>

<details>
<summary><b>6. <code>calculate_archie_saturation</code> — Archie Water Saturation ($)</b></summary>

Computes continuous water saturation ($), hydrocarbon saturation ( = 1 - S_w$), and Bulk Volume Hydrocarbon ($\text{BVH} = \phi \times (1 - S_w)$):
...
| 	op_depth | NUMBER | **Yes** | — | Top depth in meters |
| ottom_depth| NUMBER | **Yes** | — | Bottom depth in meters |
| w | NUMBER | No |  .05 | Formation water resistivity ($\Omega\cdot\text{m}$) |
| m | NUMBER | No | 2.0 | Cementation exponent |
| 
 | NUMBER | No | 2.0 | Saturation exponent |
</details>

<details>
<summary><b>7. <code>compute_permeability_timur_coates</code> — Continuous Permeability & Flow Capacity</b></summary>

Calculates continuous reservoir permeability ($ in mD) and cumulative flow capacity ( \cdot h$ in $\text{mD}\cdot\text{m}$) using the Timur or Coates empirical models:
...
Compares 4 established non-linear {\text{sh}}$ models against linear Gamma Ray index:
1. **Linear**: {\text{sh}} = I_{GR}$
2. **Larionov (Tertiary)**: {\text{sh}} = 0.083 \cdot (2^{3.7 \cdot I_{GR}} - 1)$
3. **Steiber**: {\text{sh}} = \frac{I_{GR}}{3 - 2 \cdot I_{GR}}$
4. **Clavier**: {\text{sh}} = 1.7 - \sqrt{3.38 - (I_{GR} + 0.7)^2}$
</details>

<details>
<summary><b>9. <code>compute_sonic_porosity_wyllie</code> — Wyllie Time-Average Porosity</b></summary>

Computes matrix sonic porosity from compressional acoustic transit time ($ in $\mu\text{s/ft}$) and evaluates compaction factors:
...
Scans the entire well depth series, identifies all continuous pay packages exceeding minimum thickness thresholds (default: $\ge 1.5\,\text{m}$), and ranks sweet spots by Net Pay and Hydrocarbon Pore Volume ($).
</details>

<details>
<summary><b>11. <code>plot_crossplot_picket</code> — Classic Archie Picket Plot</b></summary>

Constructs a logarithmic resistivity vs. porosity crossplot with 100% $ water line and iso-saturation lines ( = 20\%, 30\%, 50\%$) to graphically confirm formation water resistivity ($) and cementation factor ($).
</details>

<details>
<summary><b>12. <code>generate_reservoir_composite_report</code> — Executive Zonal Dossier</b></summary>

Generates an all-in-one reservoir summary table combining gross interval, net pay, NTG, average porosity, fluid saturations, flow capacity ( \cdot h$), and fluid classification.
</details>
...
<details>
<summary><b>14. <code>query_geology_metadata</code> — Stratigraphy & Mudlog Semantic RAG</b></summary>

Queries the stratigraphy catalog or Weaviate vector index for geological formations, stratigraphy tops, core descriptions, and mudlog hydrocarbon show notes.
</details>
...
- **Badge Audit Trail**: Every tool invoked by the agent renders as a clickable, expandable badge (⚙️ compute_net_pay, 🔍 Weaviate RAG) showing exact inputs and outputs.
...
- **Clastic Lithology Focus**: Matrix trendlines default to clean sandstones (.65\,\text{g/cm}^3$), limestones (.71\,\text{g/cm}^3$), and dolomites (.87\,\text{g/cm}^3$). Complex non-clastic evaporites (anhydrite, halite) or heavy mineral assemblages (pyrite, siderite) require custom matrix calibrations.
- **Archie Clean Sand Limitation**: Archie saturation equations assume non-conductive rock matrices and clean formation water. In high-clay shaly sands, excessive clay surface conductivity can cause Archie to overestimate water saturation ($). In such reservoirs, Waxman-Smits or Dual-Water models should be utilized.
...
- **Missing Caliper**: If a physical caliper curve (CALI) is missing from the LAS file, Track 1 defaults to an estimated bit size gauge baseline (.5''$ or .25''$) based on depth.
- **Missing Sonic**: 3D cluster cube plotting requires compressional sonic (DTCOMP / DT). If missing, standard regional defaults (\,\mu\text{s/ft}$) are substituted.
...
`ash
git clone https://github.com/your-username/well-log-rag-analytics.git
cd well-log-rag-analytics
`
...
The platform includes a native **FastMCP Server** (ackend/mcp_server.py) that exposes the entire deterministic petrophysical engine to external LLM clients such as **Claude Desktop**, **Cursor**, or **AI Sidecars**.
...
├── requirements.txt            # Python dependencies (lasio, fastapi, plotly, etc.)
...
│   ├── mcp_server.py           # FastMCP server for Claude Desktop / MCP clients
│   ├── petrophysics.py         # Deterministic subsurface math & Plotly figure generator
│   └── weaviate_client.py      # Optional vector database connector
│
